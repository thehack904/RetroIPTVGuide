        ...
        for attempt in range(_OVERPASS_MAX_RETRIES + 1):
            try:
                ...
                if resp.status_code == 429 and attempt < _OVERPASS_MAX_RETRIES:
                    wait = int(getattr(resp, "headers", {}).get("Retry-After", backoff))
                    logging.warning(
                ...
                # Successful fetch — clear any previously recorded error so that
                # the diagnostics check reflects the current (healthy) state.
                ...
                if attempt < _OVERPASS_MAX_RETRIES:
                    ...
                    _http = exc if isinstance(exc, requests.exceptions.HTTPError) else None
                    _OVERPASS_LAST_ERROR = {
                        "status_code": (
                            _http.response.status_code
                            if _http is not None and _http.response is not None
                            else None
                        ),
                        "lat": lat,
                        "lon": lon,
